...
class RequestManager {
    ...
    getMainPage(req, res) {
        res.render("tpl-connect-panel.ejs", {hostname: req.hostname});
    }
    ...
    connect(req, res) {
        ...
        return this.DBConnection.getUserData(userData).then((data) => {
           ...
           if(data === "WRONG_PASSWORD") {
                returned_data = {result: "KO", error: "WRONG_PASSWORD"}
    ...
    getMenu(req, res) {
        ...
        res.render("main-menu.ejs", {hostname:req.hostname});
    }

    getGamePage(req, res) {
        if(req.params.mode === "local") {
            res.render("game.ejs", {hostname: req.hostname});

        } else {
            res.render("game_online.ejs", {hostname: req.hostname});
        }
    }
    ...
    getProfile(req, res) {
        this.DBConnection.getUserProfileInfo(req.params.id).then((info) => {
            ...
            res.render("profile.ejs", {"info": info, "hostname": req.hostname});
    ...
    getOptionsPanel(req, res) { // Blocked for non-authenticated users (for now)
        ...
        res.render("options.ejs", {hostname: req.hostname});
    }
