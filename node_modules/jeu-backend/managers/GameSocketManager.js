    ...
    requestMovement(position) {
        ...
        return this.gh.getGame(this.socket.gameID).then((game) => {
            ...
            if(game.local) {
                ...
                let prev_position = game.getCurrentPlayer().position,
                    arme = game.grille.updatePosition(game.getCurrentPlayer(), [parseInt(position[0]), parseInt(position[1])], game.nb_tours);
        
                this.socket.emit("confirmationDeplacement", {"position": position, "prev_position":prev_position, "id_arme":arme,  "grille": game.getCurrentGrid()});    
            } else {
                let cur_player = game.getCurrentPlayer(),
                  prev_position = cur_player.position,
                  arme = game.grille.updatePosition(cur_player, [parseInt(position[0]), parseInt(position[1])], game.nb_tours);
               

               
               
                   this.io.sockets.in(`room-game${ game.game_id}`).emit("confirmationDeplacement", {"position": position, "prev_position": prev_position, "grille": game.getCurrentGrid(), "cur_player": cur_player, "id_arme": arme});
            }
    ...
    launchNewTurn(arme_posee) {
        ...
        this.gh.getGame(this.socket.gameID).then((game) => {
            ...
            if(game.continuationReady || game.local) {
                ...
                if(game.estEnCombat()) {
                    ...
                    if(prev_player.pv === 0) {
                        ...
                        if(!game.local) {
                            ...
                            this.DBConnection.addMatch({"player1": winner.online_id, "player2": loser.online_id, "type": "online", "winner": winner.online_id}); // On envoie les données de partie au serveur, mis online pour le moment    
                        }
                ...
                } else { // Si l'on est en combat ou qu'un des joueurs est mort
        ...
        this.gh.getGame(this.socket.gameID).then((game) => {
            if(game.local) {
                let cur_player = game.getCurrentPlayer(),
                    ...
                    damage;
                if(decision === "defense") {
                   cur_player.updatePosture(false);
               } else {
                   enemy = game.getEnemy();
                   
                   damage = enemy.recevoirDegats(cur_player.arme);

                   cur_player.updatePosture(true); // Puisqu'il a attaqué, il sort de sa posture défensive
               }
                
                let info = {"id":enemy.id, "remaining_hp": enemy.pv};

                this.io.sockets.in(`room-game${ game.game_id}`).emit("resultatDecision", info);
                ...
                if(decision === "defense") {
                   cur_player.updatePosture(false);
                    ...
                    enemy.recevoirDegats(cur_player.arme);
                   remaining_hp = enemy.pv;
                   cur_player.updatePosture(true); // Puisqu'il a attaqué, il sort de sa posture défensive
               }

                this.io.sockets.in(`room-game${ game.game_id}`).emit("resultatDecision", {"id": enemy.id, "remaining_hp": remaining_hp});
