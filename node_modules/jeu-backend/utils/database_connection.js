...
class DatabaseConnection {
    ...
    */
               ...
               if(results.length === 0) { // If the user has to be created, we call createUser
                   this.createUser(params).then((res) => {
                       if(res.username) { // We check if the username exists (the account has been properly created)
                           ...
                           this.createOptions(res.id).then(() => {
                                resolve(res);    
                           });
    ...
    createOptions(id) { // Called after creating a new user (will require a new user to test it because of the foreign key constraint)
        return new Promise((resolve, reject) => {
            this.db_connection.query(this.generateCreateOptionsQuery(id), function(err, results) {
                if(err) {
                    reject(err);
                }
    ...
    generateCreateOptionsQuery(id) {
        return `INSERT INTO options VALUES ('', ${id}, 1, .5, "fr")`;
    }
}
