...
class Grille {
    ...
    genererGrilleVide() {
        ...
        if(typeof Array.fill !== "undefined") { // If Array.fill (ES6) is implemented, we use it
            Array(11).fill(Array(11).fill("O"));

        } else { // otherwise, we use a classic grid generation
            let x = 0,
                y = 0,
                ligne = [];
    ...
    placerCasesGrisees() {
        ...
        for (let case_actuelle of cases_grisees) { // On boucle ici pour éviter d'avoir à potentiellement reparcourir tout le tableau à chaque coup
            ...
            this.applyValueToCoords("X", case_actuelle);
        }
    }
    ...
    placerJoueurs() {
        ...
        for(let nb_joueur = 0; nb_joueur < 2; nb_joueur +=1) {
            ...
            this.applyValueToCoords(String(nb_joueur+1), pos_joueurs[nb_joueur]);
        }
    }
    ...
    placerArmes() {
        ...
        for(let nb_arme = 0; nb_arme < 3; nb_arme += 1) {
            ...
            this.applyValueToCoords(String.fromCharCode(66+nb_arme), armes[nb_arme]); // Renvoie B, C, D
        }
    }
    ...
    */
    ...
    updatePosition(joueur, position, nb_tours) {
        ...
        if(this.grille[position[0]][position[1]] !== "O") { // Si on est sur une arme, on modifie la variable arme pour permettre au programme de savoir sur laquelle on a cliqué
            ...
            this.applyValueToCoords([joueur.id, this.transformerValeur(joueur.arme.id)], position); // On stocke un tableau à 2 éléments dans cette case pour pouvoir replacer l'arme au prochain passage
        ...
        if(this.grille[pos_actuelle[0]][pos_actuelle[1]] instanceof Array) { // Si le joueur avait ramassé une arme au tour précédent : (qu'on avait stocké l'id du joueur + celui de l'arme sur une case)
            ...
            this.applyValueToCoords(ancienne_arme, pos_actuelle); // Remet la case en place
